...
func batchUsersTimes(ctx context.Context, keys dataloader.Keys) []*dataloader.Result {
	fmt.Println(keys)
...
func WeeksTimes(id string, timesLoader *dataloader.Loader, ch chan []int) {
	fmt.Println("getting ", id, "times")
	resultThunk := timesLoader.Load(context.Background(), dataloader.StringKey(id))

	resultRaw, err := resultThunk()

	if err != nil {
		return nil, err
	}

	result := resultRaw.(*dataloader.Result)

	return result.Data.([]int), result.Error
		...
		"weeklyAverage": &graphql.Field{
			...
			Resolve: func(p graphql.ResolveParams) (interface{}, error) {
				return func() (interface{}, error) {
					...
					loader := p.Context.Value("loader").(*dataloader.Loader)
					...
					weeksTimes, err := WeeksTimes(user.ID, loader)
					if err != nil {
						return nil, err
					}
		...
		"weeksTimes": &graphql.Field{
			...
			Resolve: func(p graphql.ResolveParams) (interface{}, error) {
				return func() (interface{}, error) {
					...
					return WeeksTimes(value.ID, p.Context.Value("loader").(*dataloader.Loader))
				}, nil
			},
		},
	},
...
func httpHeaderMiddleware(next *handler.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ctx = context.WithValue(ctx, "loader", dataloader.NewBatchedLoader(batchUsersTimes))
